...
void InitSCT(unsigned int SystemCoreClock, unsigned int timeUnit /* in ms*/) {
   ...
   unsigned int GreenOFF = basePeriod / 10;                        // il LED VERDE si spegne poco dopo: ex. (baseperiod / 10)
   ...
   LPC_SCT->MATCH[0].U = basePeriod;     // Match0 is the AUTOLIMIT event, determines the period of the PWM
   LPC_SCT->MATCHREL[0].U = basePeriod;  // "
   LPC_SCT->MATCH[1].U = GreenON;        // green LED on, CTOUT_0/P1.0 = '0'
   LPC_SCT->MATCHREL[1].U = GreenON;     // "
   LPC_SCT->MATCH[2].U = GreenOFF;       // green LED off, CTOUT_0/P1.0 = '1'
