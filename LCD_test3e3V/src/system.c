...
#define CHECK_RSVD(val, mask)                     (val & mask)
   ...
   #error "MAINCLKSEL: Invalid values of reserved bits!"OK. Did you see my email to Krishna about the DAC0 enable crap?
#endif
...
void SystemInit (void) {
  ...
  temp &= ~(1 << FRO_DIRECT);                         // Preserve all but the bits of interest
  #if (FRO_DIRECT_VAL == 1)
    temp |= (1 << FRO_DIRECT);
  #endif
  ...
  // Or, configure it if needed for other, e.g. clock out. 
        ...
        LPC_IOCON->PIO0_8         &= (IOCON_MODE_MASK | MODE_INACTIVE);  // Disable pull-up and pull-down
        LPC_IOCON->PIO0_9         &= (IOCON_MODE_MASK | MODE_INACTIVE);  // Disable pull-up and pull-down
