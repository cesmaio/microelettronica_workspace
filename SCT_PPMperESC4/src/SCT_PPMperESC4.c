...
int main(void) {
  ...
  WriteInitial((uint8_t*)"Ciao Francesco  "); DelayMs(400);
  for(i=0; i<4;i++)
  {
	  PutCommand(DISPLAY_MOVE_SHIFT_LEFT); DelayMs(150);
  }
  DelayMs(1000);
  ...
  while(1) {
		...
		if(dummyVal < 11) 			voltage = 0;
		else 	voltage = ( 332000*dummyVal/4096+ 5)/10;// tensione di ingresso in mV
